...
def check_search(page):
    if (
        ...
        for i in page.find("ul").find_all("li"):
            url = i.find("a")
            if not "redlink=1" in url.get("href"):
                urls.append(f"{url.get('title')}\n")
        return 0, urls
...
def get_properties(properties):
    index = 0
    while properties[index].name != "h2":
        index += 1
    ...
    #         print("\t", clean(i.text))
    #     else:
    #         p = [clean(j.text) for j in i.find_all("li")]
    #         if p: print(p)
    ...
    for i in properties[:index]:
        if i.name == "h3":
            ...
            prop = [f" ❖ {clean(j.text)}" for j in i.find_all("li")]
...
def wikiphyto_api(plant_name):
    ...
    if check_code == 0:
        return suggestion, 0
    elif check_code == 1 or status_code != 200:
        return None, 1
    ...
    latin_name = (
        page.find("span", {"id": "D.C3.A9nomination_latine_internationale"}).find_next().text
    )
    ...
    if len(used_parts) > 1000:
        used_parts = used_parts[:1000] + "…"
