...
class OdysseeCommands(commands.Cog):
    ...
    async def aide(self, ctx, commande: str = None):
        ...
        if commande:
            embed = discord.Embed(
                title="Aide détaillée", description="Informations complémentaires", color=8421504
            )
            ...
            if commande in cmnd_data:
                ...
                embed.add_field(
                    name="Erreur : commande inconnue",
                    value=f"Entrez `{self.PREFIX}aide` pour avoir la liste des commandes.",
                )
            ...
            for embed in make_embed(
                fields,
                "Rubique d'aide",
                f"Entrez : `{self.PREFIX}aide <commande>` pour plus d'informations.",
            ):
                await ctx.send(embed=embed)

    @commands.command(
        help=f"Votre espèce est à préciser impérativement. Si aucun pseudonyme n'est précisé, Odyssée prendra votre nom d'utilisateur.\n__Liste des espèces connues :__ {get_all_species()}",
    ...
    async def nouveau(self, ctx, espece: str, nom: str = None):
        ...
        if not user[1] in self.data_player:
            self.data_player.update(
                {user[1]: Player(user[1], nom, espece, str(ctx.author.avatar.url))}
            )
    ...
    @commands.command(
        help="Vous permet de mettre à jour votre avatar dans les fichiers du bot si vous en changez.",
        brief="Mettre à jour son avatar",
    )
    @commands.check(server_id)
    async def avatar(self, ctx, url: str = None):
        player = get_player_from_id(ctx.author.id)
        if not player:
            await send_error(ctx, f"{ctx.author.name} n'est pas un joueur enregistré")
            return

        if not url:
            player.avatar = str(ctx.author.avatar.url)
        else:
            player.avatar = url
    ...
    @commands.command(
        help="Permet de changer son pseudo dans le jeu. Le pseudo utilisé avec Odyssée n'a aucun rapport avec celui du serveur Discord.",
        brief="Changer de pseudo",
    )
    @commands.check(server_id)
    ...
    @commands.command(
        help="Changer sa couleur. En format de couleur, vous pouvez donner le code RVB en hexadécimal ou entrer le nom de la couleur voulue (voir les noms sur le site : http://www.proftnj.com/RGB3.htm).",
        brief="Changer sa couleur",
    )
    @commands.check(server_id)
    async def couleur(self, ctx, couleur: str):
        ...
        if couleur.startswith("0x"):
            ...
            colors = {
                colors.text: f"{colors.get('value').lower()}"
                for colors in BeautifulSoup(colors, features="html5lib").find_all("option")
            }
            ...
            if couleur in colors:
                ...
                await send_error(
                    ctx,
                    f"couleur inconnue : '{couleur}'. Pour avoir la liste des couleurs disponible : http://www.proftnj.com/RGB3.htm",
                )
    ...
    @commands.command(
        help="Voir ses statistiques, spécifier un nom de joueur permet de voir les statististiques du joueur ciblé.",
        brief="Voir ses statistiques",
    )
    @commands.check(server_id)
    async def stat(self, ctx, joueur: str = None):
        if joueur:
            joueur = get_player_from_name(joueur)
        else:
            joueur = get_player_from_id(ctx.author.id)

        if not joueur:
            await send_error(
                ctx,
                "vous n'êtes pas un joueur enregistré ou le joueur dont vous voulez voir les statististiques n'existe pas",
            )
            return
        ...
        embed = discord.Embed(
            title=f"{info[0]}",
            description=f"[{info[19]}]\nStatistiques de {info[0]}, {info[1]}\n de niveau {info[2]}",
            color=info[13],
        )
        if info[17]:
            embed.set_thumbnail(url=info[17])

        # Capacities
        capacities = ""
        for index, capacity_name in enumerate(
            ("Courage ..", "Force ....", "Habileté .", "Rapidité .", "Intellect ", "Défense ..")
        ):
            capacities += f"`{capacity_name}.: {info[3 + index]}`\n"

        # Misceleanous
        misc = ""
        for index, misc_name in enumerate(
            (
                f"Vie ...#/{joueur.get_max_health()} PV",
                f"Mana ..#/{joueur.get_max_mana()}",
                "Argent # Drachmes",
                f"Poids .#/{joueur.get_max_weight()}",
            )
        ):
        ...
        if info[20]:
            abilities = ""
            for i in info[20]:
                abilities += f" ❖ {i[0].capitalize()} ({i[1]} / 20)\n"
        else:
            abilities = "< aucune >"
        ...
        embed.add_field(
            name=f"Pouvoirs ({len(joueur.power)}/{joueur.get_max_power()})",
            value=powers,
            inline=True,
        )
    ...
    @commands.command(
        help="Permet de gérer ses notes.\n\n`contenu` peut être soit une chaîne de caractères si l'on veux ajouter une note, soit l'indice de la note que l'on veut supprimer.",
        brief="Gérer ses notes",
    )
    @commands.check(server_id)
    ...
    @commands.command(
        help="Voir la liste des joueurs enregistrés", brief="Voir les joueurs enregistrés"
    )
    @commands.check(server_id)
    async def liste(self, ctx):
        ...
        for player_id in self.data_player:
            ...
            fields.append(
                (
                    f"{player.name}{('', ' (PnJ)')[player.id <= 0]}",
                    f"{player.species} de niveau {player.get_level()} vers {player.place}\n[{player.get_state()}]",
                )
            )
    ...
    @commands.command(
        help="Le premier paramètre est le nom de l'objet, le second est optionnel et correspond au nombre d'objets pris.",
    ...
    async def prend(self, ctx, nom: str, nombre: int = 1):
        ...
        if not check in (0, 3):
            ...
            await send_error(
                ctx,
                f"__{player.name}__ ne peut pas prendre l'objet : '{nom}' (objet non préhensible ou déjà possédé)",
            )

        save_game()

    @commands.command(
        help="Jetter un objet, le premier paramètre est le nom de l'objet à jeter, le second est optionnel et correspond au nombre d'objets à jeter.",
        brief="Jeter un objet",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def jette(self, ctx, nom: str, nombre: int = 1):
        ...
        if check == 1:
            ...
            await send_error(
                ctx,
                f"__{player.name}__ ne possède pas l'objet : '{nom}', ou pas en assez grande quantité",
            )

        save_game()

    @commands.command(
        help="Sert à utiliser un objet (manger de la nourriture ou consommer un bien). Le premier argument est le nom de l'objet à utiliser, le second (optionnel) correspond au nombre d'unité à utiliser.",
        brief="Utiliser un objet",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def utilise(self, ctx, nom: str, nombre: int = 1):
        ...
        if check == 1:
            ...
            await send_error(
                ctx,
                f"__{player.name}__ ne possède pas l'objet : '{nom}', ou pas en assez grande qantité",
            )

        save_game()

    @commands.command(
        help="Donner un objet à un joueur. Le premier argument est le nom du joueur, le second le nom de l'objet ('Argent', ou 'Drachmes' pour donner de l'argent). Le dernier paramètre, faculatatif correspond à la quantité donnée (nombre d'unité ou montant).",
        brief="Donner un objet",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def donne(self, ctx, joueur: str, objet: str, nombre: int = 1):
        ...
        if not target:
            await send_error(ctx, f"{joueur} n'est pas un joueur enregistré")
            return

        if player.place != target.place:
            await send_error(
                ctx, f"__{player.name}__ et __{target.name}__ ne sont pas au même endroit"
            )
            return
        ...
        if object_name in ("argent", "drachmes"):
            if player.stat[8] >= nombre:
                ...
                await ctx.send(
                    f"__{player.name}__ donne {nombre} Drachme{('', 's')[nombre > 1]} à __{target.name}__."
                )
            else:
            ...
            if not check_player:
                await send_error(
                    ctx,
                    f"__{player.name}__ ne possède pas cet objet : '{objet}' ou pas en assez grande quantité",
                )
                if check_target:
                    target.object_del(*target.have(object_name), nombre, True)
                is_ok = False

            if not check_target:
                await send_error(ctx, f"__{target.name}__ a déjà cet objet : '{objet}'")
                if check_player:
                    player.object_add(*player.have(object_name), nombre, object_name)
                is_ok = False
    ...
    @commands.command(
        name="dé",
        help="Lancer un dé. Le premier paramètre est le nombre de dés lancés, le second est le nombre de faces du ou des dés lancés (> 3). Les deux paramètres sont optionnels, par défault, un dé à 20 faces est lancé.",
        brief="Lancer un dé",
    )
    @commands.check(server_id)
    async def de(self, ctx, nombre: int = 1, faces: int = 20):
        ...
        await ctx.send(
            f"__{player.name}__ lance {nombre} dé{('', 's')[nombre > 1]} à {faces} faces : {result} / {nombre * faces}."
        )

    @commands.command(
        help=f"Effectue un lancer de dé dans dans une capacité ou une compétence.\n\n__Capacités connues :__ {', '.join(get_capacities()[: 6])}",
        brief="Effectuer un lancer dans une capacité",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def lancer(self, ctx, capacite: str):
        ...
        if not capacite in capacities:
            ...
            if check != -1:
                die_score += player.abilities[check][1] // 5
            ...
            if die_score > 15:
                ...
                if check == -1:
                    ...
                    await ctx.send(
                        f"__{player.name}__ a fait un succès critique sur son lancer de compétence : '{capacite}'"
                    )

            elif check == -1:
                await ctx.send(
                    f"__{player.name}__ n'a pas réussi à apprendre la compétence : '{capacite}'."
                )

            elif die_score >= 10:
                await ctx.send(
                    f"__{player.name}__ a fait un succès sur son lancer de compétence : '{capacite}'."
                )

            elif die_score > 5:
                await ctx.send(
                    f"__{player.name}__ a fait un échec sur son lancer de compétence : '{capacite}'."
                )
                ...
                if player.sub_abilities(capacite, check) == 1:
                    ...
                    await ctx.send(
                        f"__{player.name}__ a fait un échec critique sur son lancer de compétence : '{capacite}'."
                    )
            ...
            comment = ("échec critique", "échec", "succès", "succès critique")[
                player.capacity_roll(index)
            ]
            await ctx.send(
                f"__{player.name}__ a fait un {comment} sur son lancer "
                + ("de ", "d'")[index in (2, 4)]
                + capacities[index]
                + "."
            )
    ...
    @commands.command(
        help="Utiliser ou consulter ses pouvoirs. Le premier argument est le nom du pouvoir à utiliser, le second correspond au nom de l'adversaire visé (dans le cas où le pouvoir utilisé requiert un adversaire).\n\n Si vous laissez ses deux argments vide, vous obtenez la liste de vos pouvoirs avec une description.",
        brief="Utiliser ou consulter ses pouvoirs",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def pouvoir(self, ctx, nom: str = None, adversaire: str = None):
        ...
        if adversaire:
            ...
            if not adversaire:
                await send_error(ctx, f"{name} n'est pas un joueur enregistré")
                return
        ...
        if nom:
            ...
            await ctx.send(
                f"__{player.name}__ utilise {nom} :\n"
            ...
            for power in player.power:
                description = (
                    f"{power.description} (coût en Mana : {power.mana_cost})"
                    + ("", " (cible requise)")[power.enemy]
                )
                fields.append((power.name.capitalize(), description))

            for embed in make_embed(
                fields,
                f"Pouvoirs de {player.name}",
                "Pouvoirs spéciaux connus et descriptions",
                player.stat[10],
            ):
                await ctx.send(embed=embed)
    ...
    @commands.command(
        help="Vous permer d'apprendre de nouveau sort. le nom du sort est obligatoire.",
        brief="Apprend un nouveau pouvoir spécial",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def apprend(self, ctx, nom: str):
        ...
        if check == 0:
            ...
            await send_error(
                ctx, f"__{player.name}__ a déjà le pouvoir : '{nom}' ou vous avez trop de pouvoirs"
            )
        else:
    ...
    @commands.command(
        help="Vous permer d'oublier un sort. le nom du sort est obligatoire.",
        brief="Oublier un pouvoir spécial",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    ...
    @commands.command(
        help="Vous permet de consulter la liste des articles disponible. Préciser un nom vous renvoie la description détaillée de l'objet demandé.",
        brief="Voir les articles disponibles",
    )
    @commands.check(server_id)
    async def article(self, ctx, nom: str = None):
        ...
        if not nom and shop_id == -1:
            await send_error(ctx, f"__{player.name}__ n'est pas dans un magasin")
            return
        ...
        if nom:
            ...
            if check == -1:
                ...
                if shop_id == -1:
                    await send_error(ctx, f"__{player.name}__ ne possède pas l'objet : '{nom}'")
                    return
                if obj.shop_id == -1:
                    await send_error(ctx, f"l'objet : '{nom}' n'est pas vendu ici")
                    return

            if obj.shop_id == -1:
                shop = "Cet objet n'est rattaché à aucun magasin"
            else:
                shop = f"Magasin de rattachement : {get_shop_name()[obj.shop_id][0]}"
            ...
            misc = ""
            for index, name in enumerate(
                ("Type ..", "Vie ...# PV", "Mana ..#", "Valeur # Drachmes", "Poids .#")
            ):
                if not index:
            ...
            for embed in make_embed(
                fields, player.place, "Liste des articles disponible", player.stat[10]
            ):
                await ctx.send(embed=embed)

    @commands.command(
        help="Vous permer d'acheter des objets. Le premier paramètre est le nom de l'objet, le second, faculatatif, est le nombre d'objet à acheter.",
        brief="Acheter un objet",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def achat(self, ctx, nom: str, nombre: int = 1):
        ...
        if obj.object_type not in (1, 5, 8) or nombre < 1:
            nombre = 1
        ...
        if player.stat[8] >= nombre * obj.stat[8]:
            ...
            if check:
                ...
                await ctx.send(
                    f"__{player.name}__ a acheté l'objet : '{nom}{nb}' pour {nombre * obj.stat[8]} Drachmes."
                )
            else:
            ...
            await send_error(
                ctx, f"__{player.name}__ n'a pas assez de Drachmes pour acheter cet objet : '{nom}'"
            )

        save_game()

    @commands.command(
        help="Permet de vendre un objet. Le nom est à préciser, le nombre d'unité à vendre est facultatif (1 par défaut)",
        brief="Vendre un objet",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def vend(self, ctx, nom: str, nombre: int = 1):
        ...
        if obj.shop_id != shop_id:
            await send_error(ctx, f"l'objet : '{nom}' n'intéresse personne ici")
            return
        if obj.object_type not in (1, 5, 8) or nombre < 1:
            nombre = 1
        ...
        if check:
            ...
            player.stat[8] += cost
            await ctx.send(
                f"__{player.name}__ a vendu l'objet : '{nom}{nombre}' pour {cost} Drachmes."
            )
        else:
            await send_error(
                ctx,
    ...
    @commands.command(
        help="Vous permet de vous reposer lorsque vous dormez dehors.", brief="Dormir"
    )
    @commands.check(server_id)
    async def dormir(self, ctx):
        ...
        if player.place in places:
            await send_error(
                ctx,
                f"__{player.name}__ ne peut pas dormir : il y a des ennemis potentiels à proximité",
            )
            return
    ...
    @commands.command(
        help=f"Calculer la vitesse nécessaire pour parcourir une certaine distance.\n\n__Moyens de transport__ : {get_all_travel_mean()}\n\n__Météo__ : {get_all_weather()}\n\n__Terrains__ : {get_all_landtype()}",
        brief="Calculer un temps de trajet",
    )
    @commands.check(server_id)
    async def temps(
        self, ctx, distance: int, moyen_transport: str, meteo: str, terrain: str = None
    ):
        ...
        if -1 in (per_hour, per_day, on_sea):
            await send_error(ctx, "moyen de transport inconnu")
            return
        if weather == -1:
            await send_error(ctx, "météo inconnue")
            return
        ...
        if on_sea:
            embed = discord.Embed(
                title="Temps",
                description=f"Temps nécessaire pour parcourir {distance} km",
                color=8421504,
            )
            embed.add_field(
                name="Sur la mer",
                value=f"`jour(s) ..: {int(distance // per_day)}`\n`heure(s) .: {int(distance % per_day // per_hour)}`",
                inline=False,
            )
            await ctx.send(embed=embed)
            ...
            if not terrain:
                await send_error(ctx, "terrain inconnu")
                return
            ...
            for index, land_type in enumerate(terrain):
                embed.add_field(
                    name=land_name[index],
                    value=f"`jour(s) ..: {int(distance // (per_day * land_type))}`\n`heure(s) .: {int(distance % (per_day * land_type) // per_hour)}`",
                    inline=False,
                )
    ...
    @commands.command(
        help=f"Génère une liste de noms aléatoire selon une catégorie donnée parmi la liste : {get_categories()}.",
        brief="Génère des noms aléatoire",
    )
    @commands.check(server_id)
    async def nom(self, ctx, categorie: str = "human", nombre: int = 1):
        ...
        names = [
            names.contents[0] for names in BeautifulSoup(names, features="html5lib").find_all("li")
        ]

        embed = discord.Embed(
            title="Générateur de nom",
            description=f"Nom{('', 's')[nombre > 1]} '{categorie}' au hasard",
            color=player.stat[10],
        )
    ...
    @commands.command(
        help="Commencer ou continuer un combat. Précisez le nom de l'adversaire et l'arme utilisée.",
        brief="Combattre",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def combat(self, ctx, adversaire: str, arme: str = None):
        ...
        if not target:
            ...
            self.data_player.update(
                {new_player_id: Player(new_player_id, adversaire, "Ennemi", "", stat, player.place)}
            )
            save_game()
            ...
            if target.state == 6:
                await ctx.send(
                    "__{player.name}__ ne peut pas attaquer __{target.name}__ car ce dernier est invisible."
                )
                return
    ...
    @commands.command(
        name="plante",
        help="Donne des informations sur la plante demandée (source : wikiphyto.org)",
        brief="Informations sur une plante",
    )
    async def plante(self, ctx, nom: str):
        try:
            ...
            embed.add_field(
                ...
                value=f"Votre requête ne correspond pas à une plante ou la page correspondante n'est pas formatée de manière standard. Par conséquent Odyssée ne parvient pas à récupérer et analyser les données souhaitées.\nVous pouvez tout de même obtenir des informations en suivant le lien :\n> http://www.wikiphyto.org/wiki/{nom.replace(' ', '_')}",
            )
            await ctx.send(embed=embed)
            return
        ...
        if check_code == 0:
            embed = discord.Embed(
                title=nom,
                description=f"Plusieurs plantes correspondent à la recherche : '{nom}'",
                color=8421504,
            )
            embed.add_field(
                name="Essayez un nom de plante suivant", value=" ❖ " + " ❖ ".join(result)
            )
        ...
        # pas de résultat
            ...
            embed = discord.Embed(title=nom, description="Erreur", color=8421504)
            embed.add_field(
                name="Plante non trouvée",
                value=f"Aucune plante ne correspond à la recherche : '{nom}'",
            )
        ...
        # succès
            ...
            embed = discord.Embed(
                title=nom, description=f"Plus d'informations sur :\n> {url}", color=8421504
            )
            if img:
                embed.set_image(url=img)
            embed.add_field(
                name="Description et habitat", value="\n".join(description), inline=True
            )
            embed.add_field(name="Parties utilisées", value="\n".join(used_parts), inline=True)
            embed.add_field(
                name="Famille botanique et nom latin", value=f"{family}{latin_name}", inline=True
            )
            ...
            for i in properties:
                if len(i) > 1:
                    ...
                    embed.add_field(name=i[0], value="< aucune >", inline=False)
...
class AdminCommands(commands.Cog):
    ...
    @commands.command(
        help="Affiche la documentation sur les fonctions d'administration.",
        brief="Afficher ce panneau",
    )
    ...
    async def aide_admin(self, ctx, commande: str = None):
        ...
        if commande:
            ...
            for embed in make_embed(
                fields,
                f"Rubrique des commandes administrateur",
                f"Entrez : `{self.PREFIX}aide_admin <commande>` pour plus d'informations.",
            ):
                await ctx.send(embed=embed)

    @commands.command(
        name="joueur+",
        help="Permet d'ajouter un personnage non jouable au jeu en cours. Préciser l'espèce du joueur et son nom.",
        brief="Ajouter un PnJ",
    )
    @commands.check(is_admin)
    async def joueur_plus(
        self, ctx, nom: str, espece: str, niveau: int = 0, lieu: str = "< inconnu >"
    ):
        if get_id_from_name(nom):
            await send_error(ctx, f"le joueur : '{nom}' existe déjà")
        else:
            if niveau <= 0:
                niveau = randint(1, get_avg_level(self.data_player) + 2)
            new_player_id = min(self.data_player.keys()) - 1
            if new_player_id > 0:
                new_player_id = -1
            ...
            self.data_player.update(
                {new_player_id: Player(new_player_id, nom, espece, "", stat, lieu)}
            )
    ...
    @commands.command(
        name="joueur-",
        help="Permet de supprimer un joueur sans le kicker.",
    ...
    @commands.command(
        help=f"Permet de mofifier chaque caractéristique d'un joueur.\n\n__Caractétistiques connues :__ les capacités ({', '.join(get_capacities())}), les statistiques, l'inventaire, le lieu, les états, les pouvoirs les compétences et les notes.",
        brief="Modifier un joueur",
    )
    @commands.check(is_admin)
    async def modifier(self, ctx, nom: str, capacite: str, valeur, nombre: int = 1):
        ...
        if capacite in capacities:
            ...
            if player.state == 0 and player.stat[6] < player.get_max_health():
                player.state = 3
            elif player.state == 3 and player.stat[6] >= player.get_max_health():
                player.state = 0

            if capacite != "argent":
                msg = f"__{player.name}__ {('perd', 'gagne')[valeur > 0]} {abs(valeur)} point{('', 's')[abs(valeur) > 1]} "
                if capacite == "habileté":
                    msg += "d'Habileté"
                elif capacite == "intelligence":
                    msg += "d'Intelligence"
                else:
                    msg += f"de {capacite.capitalize()}."
            ...
            if check:
                await ctx.send(f"__{player.name}__ récupère l'objet : '{valeur}'{nombre}.")
            else:
                await send_error(ctx, f"__{player.name}__ a déjà l'objet : '{valeur}'")
            ...
            if check:
                await ctx.send(f"__{player.name}__ a perdu l'objet : '{valeur}'{nombre}.")
            else:
                await send_error(
                    ctx,
                    f"__{player.name}__ ne possède pas l'objet : '{valeur}' ou pas en assez grande quantité",
                )
            ...
            if check == 0:
                ...
                await send_error(
                    ctx,
                    f"__{player.name}__ a déjà le pouvoir : '{valeur}' ou vous avez trop de pouvoirs",
                )
            else:
            ...
            if player.add_abilities(valeur, check, nombre) == 1:
                ...
                await ctx.send(
                    f"__{player.name}__ gagne {nombre} point{('', 's')[nombre > 1]} sur la compétence : '{valeur}'."
                )
            ...
            if check == -1:
                await send_error(
                    ctx, f"__{player.name}__ ne possède pas la compétence : '{valeur}'"
                )
            else:
                if player.sub_abilities(valeur, check, nombre) == 1:
                    ...
                    await ctx.send(
                        f"__{player.name}__ a perdu {nombre} point{('', 's')[nombre > 1]} sur la compétence : '{valeur}'."
                    )
    ...
    @commands.command(
        help="Permet de modifier les statistiques d'un objet possédé par un joueur.",
        brief="Permet de modifier les caractéristique d'une arme",
    )
    @commands.check(is_admin)
        ...
        if index == -1:
            ...
            if capacite in capacities:
                ...
                if obj.object_type == 0:
                    player.stat[cap_index] += valeur

                if cap_index == 8:
                    await ctx.send(
                        f"L'objet '{objet}' de __{player.name}__ {('perd', 'gagne')[valeur > 0]} {valeur} Drachme{('', 's')[valeur > 1]} de valeur."
                    )
                else:
                    await ctx.send(
                        f"L'objet '{objet}' de __{player.name}__ {('perd', 'gagne')[valeur > 0]} {valeur} point{('', 's')[valeur > 1]} en {capacite.capitalize()}."
                    )
                ...
                if new_type:
                    ...
                    await ctx.send(
                        f"Le type de l'objet '{objet}' de __{player.name}__ est devenu : '{new_type}'."
                    )
                else:
                ...
                if 0 <= valeur < len(shops_name):
                    ...
                    await ctx.send(
                        f"Vous pouvez désormais vendre l'objet '{objet}' dans les magasins de type {new_shop}."
                    )
                elif valeur == -1:
                    player.inventory[index].shop_id = valeur
    ...
    @commands.command(
        help="Supprime la totalité de la sauvegarde du jeu en cours.",
        brief="Supprime la sauvegarde",
    )
    @commands.check(is_admin)
    ...
    @commands.command(
        help="Supprime un joueur et l'empêche de créer un nouveau personnage.",
        brief="Kick un joueur",
    )
    @commands.check(is_admin)
    ...
    @commands.command(
        help="Autorise un joueur kické à revenir. L'id du joueur est à connaître.",
        brief="Unkick un joueur",
    )
    @commands.check(is_admin)
    ...
    @commands.command(
        help="Charge la sauvegarde donnée en argument. La sauvegarde doit être en fichier joint",
    ...
    @commands.command(
        help="Sauvegarde la partie dans un fichier séparé et renvoie une copie du fichier.",
    ...
    @commands.command(
        help="Permet d'ajouter un objet au jeu. 'magagin' et 'type_objet' sont les id et non les noms.\n\n__Magasins :__\n"
        + "\n".join([f"{index} - {value[0]}" for index, value in enumerate(get_shop_name())])
        + "\n\n__Types :__\n"
        + "\n".join([f"{index} - {value}" for index, value in get_all_types()]),
        brief="Ajouter un objet",
    )
    @commands.check(is_admin)
    async def ajout_objet(
        self,
        ctx,
        magasin: int,
        type_objet: int,
        ...
        if check:
            await send_error(f"l'objet : '{nom}' existe déjà")
            return

        table = sqlite3.connect("BDD/odyssee_objects.db")
        c = table.cursor()
        c.execute(
            f'INSERT INTO objets VALUES ({magasin}, {type_objet}, "{nom}", {courage}, {force}, {habilete}, {rapidite}, {intelligence}, {defense}, {vie}, {mana}, {argent}, {poids})'
        )
    ...
    async def pnj_combat(self, ctx, joueur: str, adversaire: str, arme: str = None):
        ...
        if not pnj:
            await send_error(ctx, f"{joueur} n'existe pas")
            return
        pnj = self.data_player[pnj]
        if pnj.id > 0 or pnj.state in (2, 4):
            await send_error(
                ctx, f"__{pnj.name}__ n'est pas un PnJ ou n'est pas en état de se battre"
            )
            return
        ...
        if not target:
            await send_error(ctx, f"{adversaire} n'est pas un joueur enregistré")
            return
        target = self.data_player[target]

        if target.state == 6:
            await ctx.send(
                f"__{pnj.name}__ ne peut pas attaquer : __{target.name}__ est invisible."
            )
            return
    ...
    @commands.command(
        help="Fait dormir les joueurs (PnJ compris).", brief="Fait dormir les joueurs."
    )
    @commands.check(is_admin)
    async def nuit(self, ctx):
        ...
        if len(players_names) == 1:
            ...
            await ctx.send(
                f"{', '.join(players_names[:-1])} et {players_names[-1]} se sont reposés."
            )

        save_game()

    @commands.command(
        help="Permet de faire en sorte qu'un PnJ utilise un de ses pouvoirs",
        brief="Utiliser un pouvoir d'un PnJ",
    )
    @commands.check(is_admin)
    ...
    @commands.command(
        help=f"Permet à un PnJ de un lancer de dé dans dans une capacité ou une compétence.\n\n__Capacités connues :__ {', '.join(get_capacities()[: 6])}",
        brief="Effectuer un lancer dans une capacité pour un PnJ",
    )
    @commands.check(server_id)
    @commands.check(awareness)
    async def pnj_lancer(self, ctx, joueur: str, capacite: str):
        ...
        if pnj.id > 0 or pnj.state in (2, 4):
            await send_error(
                ctx, f"__{pnj.name}__ n'est pas un PnJ ou n'est pas en état d'effectuer un lancer'"
            )
            return
        ...
        if not capacite in capacities:
            ...
            if check != -1:
                die_score += pnj.abilities[check][1] // 5
            ...
            if die_score > 15:
                ...
                if check == -1:
                    ...
                    await ctx.send(
                        f"__{pnj.name}__ a fait un succès critique sur son lancer de compétence : '{capacite}'"
                    )

            elif check == -1:
                await ctx.send(
                    f"__{pnj.name}__ n'a pas réussi à apprendre la compétence : '{capacite}'."
                )

            elif die_score >= 10:
                await ctx.send(
                    f"__{pnj.name}__ a fait un succès sur son lancer de compétence : '{capacite}'."
                )

            elif die_score > 5:
                await ctx.send(
                    f"__{pnj.name}__ a fait un échec sur son lancer de compétence : '{capacite}'."
                )
                ...
                if pnj.sub_abilities(capacite) == 1:
                    ...
                    await ctx.send(
                        f"__{pnj.name}__ a fait un échec critique sur son lancer de compétence : '{capacite}'."
                    )
            ...
            index = capacities.index(capacite)
            comment = ("échec critique", "échec", "succès", "succès critique")[
                pnj.capacity_roll(index)
            ]
            await ctx.send(
                f"__{pnj.name}__ a fait un {comment} sur son lancer "
                + ("de ", "d'")[index in (2, 4)]
                + capacities[index]
                + "."
            )
